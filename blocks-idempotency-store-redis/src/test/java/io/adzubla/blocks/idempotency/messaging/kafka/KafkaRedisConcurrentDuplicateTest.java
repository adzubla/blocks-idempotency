...
/**
 * Re-verifies {@code KafkaIdempotencyAdviceTest}'s concurrent-duplicate
 * mechanism (Slice 037) against a real {@link RedisIdempotencyStore} instead
 * of {@code InMemoryIdempotencyStore} (Slice 041): two real threads racing
 * {@link KafkaIdempotencyAdvice#aroundIdempotentListener} on the same key,
 * over a real (Testcontainers) Redis - proving the atomicity the mechanism
 * depends on (Redis's own Lua-script guarantee, not
 * {@code ConcurrentHashMap.compute}) holds for this call path too. No
 * embedded broker is needed here, same reasoning as the in-memory version:
 * the race is at the advice/engine/store layer, not the broker's.
 */
@Testcontainers
class KafkaRedisConcurrentDuplicateTest {
    ...
    void concurrentDuplicateDeliveryOfTheSameKeyIsAckedWithoutInvokingTheListener() throws Throwable {
        ...
        RedisIdempotencyStore store = new RedisIdempotencyStore(redisTemplate, new ObjectMapper(), "idempotency-kafka-race-test:");
        ...
        KafkaTemplate<Object, Object> deadLetterTemplate = mock(KafkaTemplate.class);
        KafkaIdempotencyAdvice advice = new KafkaIdempotencyAdvice(engineRegistry, properties,
                new KafkaDeadLetterPublisher(deadLetterTemplate, ".DLT"));
    ...
    private ProceedingJoinPoint baseJoinPointFor(String idempotencyKey, String body) throws NoSuchMethodException {
        ...
        ConsumerRecord<String, String> record = new ConsumerRecord<>("orders", 0, 0L, ConsumerRecord.NO_TIMESTAMP,
                TimestampType.NO_TIMESTAMP_TYPE, ConsumerRecord.NULL_SIZE, ConsumerRecord.NULL_SIZE, "record-key", body, headers,
                Optional.empty());
